...
from gps_waypoint_follower.utils.gps_utils import latLonYaw2Geopose
...
class GpsWpCommander():
    """
    Class to use gps waypoint follower to follow a set of waypoints logged in a yaml file
    """
...
def main():
    ...
    default_yaml_file_path = os.path.join(get_package_share_directory(
        "gps_waypoint_follower"), "config", "demo_waypoints.yaml")
    if len(sys.argv) > 1:
